...
import 'package:petdays/domain/repository/medical_repository.dart';
...
class MedicalProvider extends StateNotifier<MedicalState> with LocatorMixin {
  ...
  Future<void> updateMedical({
    ...
    try {
      ...
      // 수정된 진료기록 가져오기
      MedicalModel updatedMedical = await read<MedicalRepository>().updateMedical(
        medicalId: medicalId,
        uid: uid,
        petId: petId,
        files: files,
      ...
      List<MedicalModel> updatedList = state.medicalList.map((medical) {
        ...
        ..sort((a, b) => b.visitedDate.compareTo(a.visitedDate)); // visitDate 기준으로 내림차순 정렬
  ...
  Future<void> deleteMedical({
    ...
    try {
      ...
      state = state.copyWith(medicalStatus: MedicalStatus.error); // 문제가 생기면 error로 상태 변경
  ...
  Future<void> getMedicalList({
    ...
    try {
      ...
      List<MedicalModel> medicalList = await read<MedicalRepository>().getMedicalList(uid: uid);
  ...
  Future<void> uploadMedical({
    ...
    try {
      state = state.copyWith(medicalStatus: MedicalStatus.submitting); // 게시글을 등록하는 중인 상태로 변경
